  ...
  return (
    <main className="bg-ivory-50">
      <section className="mx-auto max-w-[1600px] px-4 pb-4 pt-4 sm:px-6 lg:px-8">
        <div className="grid min-h-[calc(100svh-7rem)] grid-cols-1 lg:grid-cols-[0.86fr_1.14fr]">
          <div className="flex flex-col justify-between bg-ink-900 px-7 py-9 text-ivory-50 sm:px-12 sm:py-12 lg:px-16 lg:py-16">
          ...
          <div className="group relative min-h-[55vh] overflow-hidden bg-ivory-200 lg:min-h-0">
            <Image
      ...
      <section className="mx-auto max-w-7xl px-4 py-24 sm:px-6 sm:py-32 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[0.7fr_1.3fr] lg:gap-20">
          <div>
            ...
            <h2 className="mt-5 max-w-sm font-serif text-4xl leading-[0.98] tracking-[-0.04em] text-ink-900 sm:text-6xl">
              Pieces that hold their own.
            </h2>
            <p className="mt-7 max-w-sm text-sm leading-7 text-ink-600">
              A considered edit of our most-worn silhouettes. Designed with
              enough presence for the room, and enough ease for the morning
              after.
            </p>
            <Link
              href="/shop"
              className="link-underline mt-9 inline-flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.16em] text-ink-900"
            >
              Shop all pieces <ArrowUpRight className="h-4 w-4" />
            </Link>
          </div>

          <div className="grid grid-cols-2 gap-x-4 gap-y-14 sm:gap-x-6">
            {featured.map((product, index) => (
              <div key={product.id} className={index === 1 ? 'sm:mt-20' : ''}>
      ...
      >
        <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-24">
          <div className="relative aspect-[4/5] overflow-hidden bg-ivory-200">
            <Image
              src={editorialImages.atelier}
              alt="A jeweler inspecting a pearl at the workbench"
              fill
              className="object-cover"
              sizes="(min-width: 1024px) 55vw, 100vw"
            />
            <div className="absolute bottom-0 left-0 right-0 bg-ink-900/80 px-6 py-5 text-ivory-50 backdrop-blur-sm">
              <p className="eyebrow text-gold-300">The atelier, Bengaluru</p>
          ...
          <div>
            <p className="eyebrow text-gold-700">A slower standard</p>
            <h2 className="mt-5 font-serif text-4xl leading-[0.98] tracking-[-0.04em] text-ink-900 sm:text-6xl">
              Every detail has a reason.
            </h2>
            <p className="mt-7 max-w-lg text-base leading-8 text-ink-600">
              We begin with a wax maquette, then keep refining the silhouette
              until it feels inevitable. The result is jewelry with a point of
              view: tactile, balanced, and made to be worn into your own story.
            </p>

            <div className="mt-10 border-y border-ivory-200 py-6">
              <p className="font-serif text-2xl text-ink-900">
                Sculptural forms, softened by the hand.
              </p>
              <p className="mt-3 text-sm leading-7 text-ink-600">
                We edit every silhouette until the proportion feels right — a
                quiet balance of line, weight, and light.
              </p>
            </div>

            <Link
              href="/#craft"
              className="link-underline mt-9 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-ink-900"
            >
              Explore the complete edit <ArrowUpRight className="h-4 w-4" />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-ivory-200 bg-ivory-100/60 py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            <div>
              <p className="eyebrow text-gold-700">Fresh from the bench</p>
              <h2 className="mt-4 font-serif text-4xl tracking-[-0.04em] text-ink-900 sm:text-6xl">
                New, but already familiar.
              </h2>
            </div>
            <Link
              href="/shop?sort=newest"
              className="link-underline inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-ink-900"
            >
              See the latest <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
          <div className="mt-14 grid grid-cols-2 gap-x-4 gap-y-14 sm:gap-x-6 lg:grid-cols-4">
      ...
      <section className="mx-auto max-w-5xl px-4 py-24 text-center sm:px-6 sm:py-32">
        ...
        <blockquote className="mx-auto mt-8 max-w-4xl font-serif text-3xl leading-tight tracking-[-0.03em] text-ink-900 sm:text-5xl">
          “The kind of jewelry that makes a simple white shirt feel considered.”
        </blockquote>
