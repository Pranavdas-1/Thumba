...
function ElapsedTime({ createdAt }: { createdAt: string }) {
  ...
  return <span className="font-mono text-xs text-navy-600" aria-label={`Elapsed time ${now ? formatElapsed(now - new Date(createdAt).getTime()) : "loading"}`}>{now ? formatElapsed(now - new Date(createdAt).getTime()) : "--:--:--"}</span>;
}
